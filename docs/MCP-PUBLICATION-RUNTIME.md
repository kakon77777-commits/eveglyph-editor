...
### `validate_document`
Runs non-destructive preflight validation for profile, delimiters, fences/blocks, and Markdown → Typst conversion.
...
1. **Runtime verification pending.** The current ChatGPT container cannot resolve GitHub/npm hosts, the YepCode runner returned authorization 403, and this repository produced no GitHub Actions runs during the implementation session. Tests exist but have not been honestly observed GREEN in a network-enabled runner.
