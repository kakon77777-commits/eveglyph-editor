...
- **World Studio draft generation** — the **Studio** tab asks the configured cloud AI for a bounded state-machine draft containing states, variables, events, language instructions, responses, and transitions. The result is parsed and validated locally before it can be applied to the editor; it never writes Runtime State or saves a file automatically.
